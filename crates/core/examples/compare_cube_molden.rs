//! Same validation as `compare_cube.rs`, sourced from a `.molden` file
//! instead of `.fchk` — evaluates the same MO ourselves at every point of
//! a real reference `.cube` (PySCF's own `cubegen`, independent of our
//! parser) and reports quantitative agreement.

use std::env;
use std::path::PathBuf;

use apost3dview_core::units::ANGSTROM_PER_BOHR;
use apost3dview_core::{evaluate_mo, parse_cube, parse_molden_wavefunction};
use glam::DVec3;

fn main() {
    let args: Vec<String> = env::args().collect();
    if args.len() < 4 {
        eprintln!("usage: compare_cube_molden <path.molden> <reference.cube> <HOMO|LUMO|#N (1-based)> [ALPHA|BETA]");
        std::process::exit(1);
    }
    let molden_path = PathBuf::from(&args[1]);
    ...
    let wfn = parse_molden_wavefunction(&molden_path).expect("failed to parse .molden wavefunction");
    let reference = parse_cube(&cube_path).expect("failed to parse reference .cube");

    let orbitals = match spin.as_str() {
        "ALPHA" => &wfn.alpha,
        "BETA" => wfn.beta.as_ref().expect("this .molden has no beta orbitals (restricted calculation)"),
